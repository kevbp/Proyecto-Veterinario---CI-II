...
class AdopcionController extends Controller
{
    ...
    #[OA\Get(
        ...
        security: [['bearerAuth' => []]],
        parameters: [
            new OA\Parameter(name: 'campania_id', in: 'query', required: false, schema: new OA\Schema(type: 'string', format: 'uuid')),
        ...
        responses: [
            new OA\Response(
                ...
                content: new OA\JsonContent(
                    type: 'array',
                    items: new OA\Items(ref: '#/components/schemas/Adopcion')
                )
            ),
        ]
    )]
    ...
    {
        $filters = $request->only(['campania_id', 'fecha_inicio', 'fecha_fin']);
        $adopciones = $this->adopcionService->getAll($filters);

        return response()->json(\App\Http\Resources\AdopcionResource::collection($adopciones)->response()->getData(true));
    }

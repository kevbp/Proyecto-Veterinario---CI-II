...
#[OA\Schema(
    schema: 'StoreVacunaAnimalRequest',
    ...
    properties: [
        ...
        new OA\Property(property: 'cantidad', type: 'numeric', example: 1),
...
class StoreVacunaAnimalRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'esquema_vacuna_id' => 'required|uuid|exists:esquema_vacunas,id',
            'medicamento_id' => 'required|uuid|exists:medicamentos,id',
            'consulta_id' => 'nullable|required_without:animal_id|uuid|exists:consultas,id',

            'campania_id' => [
                'nullable',
                'uuid',
                Rule::exists('campanias', 'id')->where(function ($query) {
                    $query->where('estado', EstadoCampania::EN_CURSO->value);
                }),
            ],
        ];
    }
    ...
    public function messages(): array
    {
        return [
            ...
            'campania_id.exists' => 'La campaña seleccionada no existe o no está en curso.',
        ];
    }
}

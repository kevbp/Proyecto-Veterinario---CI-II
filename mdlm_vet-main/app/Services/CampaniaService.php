...
class CampaniaService implements CampaniaServiceInterface
{
    ...
    {
        return Campania::with('responsable')
            ->withCount(['vacunas', 'desparasitaciones'])
    ...
    public function update(string $id, UpdateCampaniaDTO $dto): Campania
    {
        ...
        $campania->update([
            ...
            'estado' => $dto->estado ?? $campania->estado,
        ]);
    ...
    public function iniciarCampania(string $id): Campania
    {
        ...
        if ($campania->estado !== EstadoCampania::PLANIFICADA) {
            throw new Exception("La campaña no se puede iniciar porque su estado actual es: {$campania->estado->value}");
        }
    ...
    public function finalizarCampania(string $id, FinalizarCampaniaDTO $dto): Campania
    {
        return DB::transaction(function () use ($id, $dto) {
            ...
            // PREVENCIÓN DE DEADLOCKS: Ordenar por medicamento_id para que el 
            // lockForUpdate() de inventarioService se adquiera siempre en el mismo orden.
            $insumos = collect($dto->insumos_consumidos)
                ->sortBy('medicamento_id')
                ->values()
                ->all();
